...
class RegisterScriptsController extends GetxController
    with StateMixin, GetSingleTickerProviderStateMixin {
  late ElementService elementRep = ElementService();
  late CardService cardRep = CardService();
  late ScriptService scriptRep = ScriptService();
  ...
  RxList<Cards> selected = RxList<Cards>();
  ...
  void _loadElements() async {
    ...
    try {
      ...
      var cards = await cardRep.findAllCards();
      if (elements != null) {
        elements.shuffle();
  ...
  void _loadWidgets({List<Elements>? questions, List<Cards>? answers}) async {
  ...
  _filterElements(TextEditingValue value) {
    if (page.value == 0) {
      ...
      return answers
          .where((i) =>
              i.title!.text.toUpperCase().contains(value.text.toUpperCase()))
          .toList();
    }
  }
  ...
  List<List<Widget>> _getCards(List<dynamic> elements, Function placeholder) {
  ...
  saveCard() async {
    if (question.value != null && selected.length > 1) {
      Scripts s = await scriptRep.postScript(
          script: Scripts(
              id: 1,
              date: "",
      ...
      Get.back(result: s, closeOverlays: true);
    } else {
